...
def main(
    ...
    # Look for metadata DB in relative path: ./data/boldigger3_db/
    database_path = Path.cwd() / "data" / "boldigger3_db"

    try:
        metadata_db_path = next(database_path.glob("*.duckdb"))
    except StopIteration:
        print(
            f"{datetime.datetime.now():%H:%M:%S}: ERROR: No .duckdb file found in {database_path}. "
            f"Make sure metadata was downloaded successfully."
        )
        return
